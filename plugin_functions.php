...
function p1xtr_image_merchandise_store_plugin_loaded($pluginName){
    if (is_admin()) {
...
function p1xtr_image_merchandise_store_activate($pluginName){
    global $p1xtr_image_merchandise_store_fields;
    $default = array();
    foreach ($p1xtr_image_merchandise_store_fields as $conf)
...
function p1xtr_image_merchandise_store_activation_redirect($plugin, $pluginName)
{
    if ($plugin == plugin_basename(__FILE__))
        exit(wp_redirect(admin_url('admin.php?page='.$pluginName.'_plugin')));
}

function p1xtr_image_merchandise_store_show_plugin($pluginName)
{
    global $post;
...
function p1xtr_image_merchandise_store_inline_script($pluginName){
    $apiKey = get_option( $pluginName . '_user');
    if (!empty($apiKey) && !wp_script_is($pluginName.'_inline', 'done') && p1xtr_image_merchandise_store_show_plugin($pluginName)) {
        $options = (object)get_option($pluginName.'_options');
...
<script src="https://pixter-loader-assets.s3.amazonaws.com/Loader/v3Loader.js?cbr=$random" onload="pLoader.initStore('$token','$storeId',function(){console.log('wp2 READY');},
    {
        isGuest: $isGuest,
        platform : 'wp',
...
function p1xtr_image_merchandise_store_register_notice($pluginName, $pluginDisplayName){
...
function p1xtr_image_merchandise_store_psk_notice($pluginName, $pluginDisplayName){
    $options = (object)get_option($pluginName.'_options');
    $toggle_psk = $options->toggle_psk;
    global $image_merchandise_store_admin_tools;
    if ($image_merchandise_store_admin_tools->getToggleBoolValue($toggle_psk) === false) {
