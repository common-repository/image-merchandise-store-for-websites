...
function p1xtr_image_merchandise_store_admin_page_register($pluginName,$pluginTitle)
{
...
registerAction = 'register_' + '$pluginName'+'_user';
    ...
    $image_merchandise_store_is_guest_user = get_option('image_merchandise_store_is_guest_user');

    $hideRegPage = '';
    if(empty($image_merchandise_store_is_guest_user) || $image_merchandise_store_is_guest_user == true ){
        $hideRegPage = 'hidden';
    }
...
function p1xtr_image_merchandise_store_admin_before_page()
{
    $logo = plugins_url('logo.png', __FILE__);
    echo "
			<style>
...
$p1xtr_image_merchandise_store_fields = array(
    array(
        "type" => "cond",
        "name" => __("Turn Pixter.me for Merchandise plugin:", 'pixter-me'),
...
class p1xtr_image_merchandise_store_admin_tools{
    private $pluginName;
    private $plugin_db_ver = 1;
    private $plugin_ver = '1.4';
    ...
    private function update_routine_1(){ // db update #1, Version 1.10 -> 1.11
        $currentApiKey = get_option($this->pluginName . '_user');
        if(empty($currentApiKey)){ // User is not registered
